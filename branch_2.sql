CREATE DATABASE LINK STORAGE CONNECT TO TROFIV IDENTIFIED BY passwd USING
  '(DESCRIPTION=
    (ADDRESS=
     (PROTOCOL=TCP)
     (HOST=0.oracle.trofiv.io)
     (PORT=1521))
    (CONNECT_DATA=
     (SID=ORCLCDB)))';
...
CREATE TABLE GOODS_TO_PROMOTIONS (
  ...
  CONSTRAINT GOOD_TO_PROMOTION_GOOD_FK FOREIGN KEY (GOOD_ID) REFERENCES GOODS (GOOD_ID),
  CONSTRAINT GOOD_TO_PROMOTION_PROMOTION_FK FOREIGN KEY (PROMOTION_ID) REFERENCES PROMOTIONS (PROMOTION_ID)
);
...
CREATE TABLE REALIZATIONS (
  ...
  CONSTRAINT REALIZATION_GOOD_FK FOREIGN KEY (GOOD_ID) REFERENCES GOODS (GOOD_ID),
  CONSTRAINT REALIZATION_PROMOTION_FK FOREIGN KEY (PROMOTION_ID) REFERENCES PROMOTIONS (PROMOTION_ID)
);